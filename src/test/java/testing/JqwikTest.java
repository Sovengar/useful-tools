...
class JqwikTest {
    ...
    @Property
    void lengthOfConcatenatedStringIsSumOfLengths(@ForAll @NotEmpty String a, @ForAll @NotEmpty String b) {
        String combined = a + b;
        assertThat(combined).hasSize(a.length() + b.length());
    }
    ...
    @Property
    void absoluteValueIsAlwaysNonNegative(@ForAll int value) {
    ...
    @Property
    void stringContainsItself(@ForAll String value) {
        assertThat(value).contains(value);
    }

    @Property
    void percentageShouldStayInValidRange(@ForAll @IntRange(min = 0, max = 100) int percentage) {
